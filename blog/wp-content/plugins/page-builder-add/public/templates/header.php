...
<?php if ($isShortCodeTemplate == true) {
  ...
  if (class_exists('E_Clean_Exit') ) {
      if (! did_action( 'wp_head' ) ) { wp_head();   $loadWpHead = 'true'; }  
  }
...
?>
  ...
  <meta name="og:description" content="<?php echo $pageSeoDescription; ?>">
  <meta name="og:image" content="<?php echo $data['pageOptions']['pageSeofbOgImage']; ?>">
...
?>
  ...
  <?php
  $checkInViewElScript = ob_get_contents();
  ob_end_clean();
  $landingpagejQueryloadScripts = $landingpagejQueryloadScripts . $checkInViewElScript;
  ?>
